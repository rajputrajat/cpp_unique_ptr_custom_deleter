...
int main()
{
    ...
    const auto data = std::shared_ptr<Data<int>>(new Data<int>(10), del);
    const auto data_ref_capturing_lam = std::shared_ptr<Data<int>>(new Data<int>(10), del_with_ref_capture);
    const auto data_val_capturing_lam = std::shared_ptr<Data<int>>(new Data<int>(10), del_with_val_capture);
    const auto data_fp = std::unique_ptr<Data<int>, void(*)(Data<int>*)>(new Data<int>(20), &del_fn);

    std::cout << "size of shared_ptr with lambda custom-deleter: " << sizeof(data) << std::endl;
    std::cout << "size of shared_ptr with lambda (ref-capture) custom-deleter: " << sizeof(data_ref_capturing_lam) << std::endl;
    std::cout << "size of shared_ptr with lambda (val-capture) custom-deleter: " << sizeof(data_val_capturing_lam) << std::endl;
    std::cout << "size of shared_ptr with function-ptr custom-deleter: " << sizeof(data_fp) << std::endl;
    return 0;
}
